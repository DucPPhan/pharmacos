    ...
    <div className="user-profile-card-body">
      ...
      <div className="user-profile-edit-btn-wrap" style={{ gap: 16 }}>
        ...
        {isFirebaseUser && (
          ...
          >
            Profile information is managed by Google
          </div>
        )}
      </div>
    </div>
...
const EditProfileInlineForm: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div
        className="user-profile-card-body"
...
const UserProfile: React.FC = () => {
  ...
  // Function to create UserInfo from Firebase user
  const createUserInfoFromFirebase = (firebaseUser: any): UserInfo => {
    return {
  ...
  useEffect(() => {
    ...
    // If user is logged in with Firebase, use Firebase data
    if (currentUser) {
      const firebaseUserData = createUserInfoFromFirebase(currentUser);
      setUser(firebaseUserData);
      setLoading(false);
    } else {
      // Fallback to API call for regular login users
      fetchProfileByRole()
        .then((data) => {
          setUser(data);
          setLoading(false);
  ...
  const handleMenuClick = (e: any) => {
    if (e.key === "logout") {
      Modal.confirm({
        ...
        onOk: async () => {
          try {
            // Logout from Firebase if user is logged in with Firebase
            if (currentUser) {
              await logout();
            }
            // Clear localStorage
            ...
            // Still navigate to home even if Firebase logout fails
            navigate("/");
          }
        },
      });
  ...
  const handleSaveProfile = async (values: UserInfo) => {
    // Firebase users should not reach this function since Edit button is hidden
    setLoading(true);
    try {
